    ...
    <main className="min-h-screen bg-[#050505] px-6 py-10 text-white">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-6 grid gap-5 md:grid-cols-3">
          <SummaryCard label="Total Analyses" value={String(totalAnalyses)} />
          <SummaryCard label="Visible Results" value={String(filteredItems.length)} />
          <SummaryCard label="Storage" value="Private account history" />
        </section>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-6 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl">
            ...
            <h2 className="text-2xl font-bold">Remove this saved analysis?</h2>
            ...
            <div className="mt-5 rounded-2xl border border-zinc-800 bg-black p-4">
              <div className="flex flex-wrap items-center gap-2">
                <Tag>{deleteTarget.market}</Tag>

                {deleteTarget.instrument && (
                  <Tag highlight>{deleteTarget.instrument}</Tag>
                )}
              </div>
    ...
    <article className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6 transition hover:border-zinc-600">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-zinc-500">{item.trade_duration}</p>
        </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <ScoreCard type="bullish" score={item.analysis?.bullishScore || 0} />
        <ScoreCard type="bearish" score={item.analysis?.bearishScore || 0} />
      </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <TextPanel
          ...
          value={item.analysis?.finalDecision || "No final decision available."}
        />
      </div>
    </article>
  );
...
function TextPanel({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-3xl border border-zinc-800 bg-black p-5">
      ...
      <p className="leading-8 text-zinc-300">{value}</p>
    </div>
  );
}
