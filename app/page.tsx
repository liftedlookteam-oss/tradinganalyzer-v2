      ...
      <main className="min-h-screen bg-[#050505] px-6 py-8 text-white">
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <p className="mb-2 text-sm font-semibold uppercase tracking-[0.3em] text-zinc-500">Analysis Result</p>
              <h1 className="text-4xl font-bold">Trade Decision Dashboard</h1>
            </div>

            <div className="flex items-center gap-3">
              <a href="/history" className="rounded-2xl border border-zinc-700 bg-zinc-950 px-5 py-3 text-sm font-bold text-zinc-200 transition hover:border-white hover:text-white">
                History
              </a>

              <button onClick={handleNewAnalysis} className="rounded-2xl border border-zinc-700 bg-zinc-950 px-5 py-3 text-sm font-bold text-zinc-200 transition hover:border-white hover:text-white">
                New Analysis
              </button>
            </div>
          </div>
          ...
          <section className="mb-6 grid gap-6 lg:grid-cols-2">
            <ScenarioPanel type="bullish" title="Bullish Scenario" scenario={analysis.bullishScenario} conditions={analysis.bullishConditions} score={analysis.bullishScore} />
            <ScenarioPanel type="bearish" title="Bearish Scenario" scenario={analysis.bearishScenario} conditions={analysis.bearishConditions} score={analysis.bearishScore} />
          </section>

          <section className="mb-6 rounded-3xl border border-yellow-500/40 bg-yellow-500/10 p-7">
            <p className="mb-2 text-sm font-bold uppercase tracking-[0.25em] text-yellow-300">Most Important Thing Right Now</p>
            <p className="text-2xl font-bold leading-tight text-white">{analysis.mostImportantThing}</p>
          </section>
          ...
          {analysis.tradeQuality === "No Trade" && analysis.noTradeReason && (
            <section className="mb-6 rounded-3xl border border-red-500/40 bg-red-500/10 p-7">
              <p className="mb-2 text-sm font-bold uppercase tracking-[0.25em] text-red-300">No Trade Reason</p>
              <p className="text-2xl font-bold leading-tight text-white">{analysis.noTradeReason}</p>
            </section>
          )}

          <section className="rounded-3xl border border-zinc-800 bg-zinc-950 p-7">
            <p className="mb-2 text-sm font-bold uppercase tracking-[0.25em] text-zinc-500">Final Decision</p>
            <p className="text-2xl font-bold leading-tight text-white">{analysis.finalDecision}</p>
          </section>
        </div>
      </main>
    );
    ...
    <>
      <main className="min-h-screen bg-[#050505] px-6 py-10 text-white">
        <div className="mx-auto max-w-7xl">
          <header className="mb-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {isPro ? (
                ...
                <button onClick={() => setShowUpgradeModal(true)} className="rounded-2xl bg-white px-5 py-3 text-sm font-bold text-black transition hover:bg-zinc-200">
                  Upgrade
                </button>
              )}
          ...
          <section className="mb-10 rounded-3xl border border-zinc-800 bg-gradient-to-br from-zinc-950 to-zinc-900 p-8 shadow-2xl">
            <div className="grid gap-8 lg:grid-cols-[1fr_420px] lg:items-center">
              <div className="max-w-4xl">
                <p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-zinc-500">AI Trading Decision Support</p>
                <h1 className="text-5xl font-bold tracking-tight">Chart Setup Analyzer</h1>
              ...
              <div className="flex items-center justify-center">
                <img src="/logo.png" alt="Logo" className="max-h-[340px] w-full object-contain" />
              </div>
            </div>
          </section>
          ...
          <section className="mb-6 rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-zinc-400">Upload at least 2 timeframes to run the analysis.</p>
              </div>

              <div className="flex flex-wrap gap-2 md:justify-end">
                {markets.map((item) => (
                  <button key={item} onClick={() => setMarket(item)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${market === item ? "bg-white text-black" : "bg-black text-zinc-400 hover:text-white"}`}>
                    {item}
                  </button>
                ))}
              </div>
          ...
          <section className="mb-6 rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
            <div className="grid gap-3 md:grid-cols-5">
              {tradeDurations.map((item) => (
                <button key={item.value} onClick={() => setTradeDuration(item.value)} className={`rounded-2xl border px-4 py-4 text-left transition ${tradeDuration === item.value ? "border-white bg-white text-black" : "border-zinc-800 bg-black text-zinc-300 hover:border-zinc-500"}`}>
                  <p className="font-bold">{item.label}</p>
                  <p className={`mt-1 text-sm ${tradeDuration === item.value ? "text-zinc-700" : "text-zinc-500"}`}>{item.description}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="mb-6 rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
            <input value={instrument} onChange={(event) => setInstrument(event.target.value)} placeholder="Instrument / Pair / Ticker (optional)" className="w-full rounded-2xl border border-zinc-800 bg-black px-5 py-4 text-white outline-none placeholder:text-zinc-600 focus:border-white" />
          </section>

          <section className="grid gap-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {timeframes.map((timeframe) => (
              <UploadBox key={timeframe.key} title={timeframe.title} description={timeframe.description} file={files[timeframe.key]} onChange={(file) => handleFileChange(timeframe.key, file)} />
            ))}
          </section>

          <button onClick={handleAnalyze} disabled={!canClickAnalyze} className={`mt-8 w-full rounded-2xl px-6 py-5 text-lg font-bold transition ${canAnalyze || isPro ? "bg-white text-black hover:bg-zinc-200" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700"}`}>
            {loading
              ? "Analyzing..."
              : isPro || canAnalyze
              ? uploadedCount >= 2
      ...
      {showUpgradeModal && !isPro && (
        <UpgradeModal onClose={() => setShowUpgradeModal(false)} onWeekly={() => startCheckout("weekly")} onMonthly={() => startCheckout("monthly")} />
      )}
    </>
  );
}

function UpgradeModal({ onClose, onWeekly, onMonthly }: { onClose: () => void; onWeekly: () => void; onMonthly: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-6 backdrop-blur-sm">
      <div className="w-full max-w-3xl overflow-hidden rounded-[2rem] border border-zinc-800 bg-[#070707] shadow-2xl">
        <div className="border-b border-zinc-800 bg-gradient-to-br from-zinc-950 to-black p-8">
          <p className="mb-3 text-sm font-bold uppercase tracking-[0.3em] text-zinc-500">ChartSetup Pro</p>
          <h2 className="text-4xl font-bold leading-tight">
            <span className="bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">Analyze without limits.</span>
          </h2>
          <p className="mt-4 max-w-xl leading-8 text-zinc-400">
            Upgrade to unlock unlimited AI chart analyses, complete history access, and priority processing.
          </p>
        </div>

        <div className="grid gap-4 p-6 md:grid-cols-2">
          <button onClick={onWeekly} className="rounded-3xl border border-white bg-white p-6 text-left text-black transition hover:bg-zinc-200">
            <p className="text-sm font-bold uppercase tracking-[0.25em] text-zinc-600">Weekly</p>
            <div className="mt-4 flex items-end gap-2">
              <p className="text-4xl font-bold">€4.99</p>
              <p className="mb-1 text-zinc-600">/ week</p>
            </div>
            <p className="mt-4 leading-7 text-zinc-700">
              Flexible access for traders who want unlimited analysis without a monthly commitment.
            </p>
            <div className="mt-6 rounded-2xl bg-black px-4 py-3 text-center font-bold text-white">Choose Weekly</div>
          </button>

          <button onClick={onMonthly} className="relative rounded-3xl border border-white bg-white p-6 text-left text-black transition hover:bg-zinc-200">
            <div className="absolute right-5 top-5 rounded-full bg-black px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] text-white">Best Value</div>
            <p className="text-sm font-bold uppercase tracking-[0.25em] text-zinc-600">Monthly</p>
            <div className="mt-4 flex items-end gap-2">
              <p className="text-4xl font-bold">€14.99</p>
              <p className="mb-1 text-zinc-600">/ month</p>
            </div>
            <p className="mt-4 leading-7 text-zinc-700">Best for active traders who analyze setups regularly.</p>
            <div className="mt-6 rounded-2xl bg-black px-4 py-3 text-center font-bold text-white">Choose Monthly</div>
          </button>
        </div>

        <div className="border-t border-zinc-800 px-6 py-5">
          <button onClick={onClose} className="w-full rounded-2xl border border-zinc-800 px-5 py-3 text-sm font-bold text-zinc-400 transition hover:border-zinc-600 hover:text-white">
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#050505] px-6 text-white">
      <section className="w-full max-w-2xl rounded-3xl border border-zinc-800 bg-zinc-950 p-8 shadow-2xl">
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-zinc-500">Analysis Running</p>
        <h1 className="text-4xl font-bold">Analyzing your setup</h1>
        <p className="mt-4 text-lg text-zinc-400">Do not close this page. Your chart context is being processed.</p>
        ...
        <div className="mt-8 space-y-4">
          {loadingMessages.map((item, index) => {
            ...
            return (
              <div key={item} className={`flex items-center gap-4 rounded-2xl border px-5 py-4 transition ${active ? "border-white bg-white text-black" : completed ? "border-zinc-700 bg-black text-zinc-300" : "border-zinc-800 bg-black text-zinc-600"}`}>
                <div className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-bold ${active ? "bg-black text-white" : completed ? "bg-white text-black" : "bg-zinc-900 text-zinc-600"}`}>
                  {completed ? "✓" : index + 1}
                </div>
